import { expect, test } from "@playwright/test";

test.describe("form validation", () => {
  test("contact empty submission stays invalid", async ({ page }) => {
    await page.goto("/contact");
    await page.getByRole("button", { name: /Envoyer le message/i }).click();

    await expect(page.locator('input[name="name"]')).toHaveJSProperty(
      "validity.valid",
      false,
    );
  });

  test("contact honeypot submission is rejected", async ({ page }) => {
    await page.goto("/contact");

    await page.locator('input[name="name"]').fill("Alaeddine BEN RHOUMA");
    await page.locator('input[name="email"]').fill("contact@qantara-ai.com");
    await page.locator('input[name="subject"]').fill("Diagnostic IA");
    await page
      .locator('textarea[name="message"]')
      .fill(
        "Nous souhaitons cadrer une démarche IA responsable avec des livrables clairs.",
      );
    await page.locator('input[name="consent"]').check();
    await page.locator('input[name="honeypot"]').evaluate((input) => {
      (input as HTMLInputElement).value = "bot-value";
    });
    await page.getByRole("button", { name: /Envoyer le message/i }).click();

    await expect(page.getByText("Certains champs doivent être corrigés.")).toBeVisible();
  });

  test("diagnostic without consent stays invalid", async ({ page }) => {
    await page.goto("/diagnostic-ia");
    await page.getByRole("button", { name: /Envoyer la demande/i }).click();

    await expect(page.locator('input[name="consent"]')).toHaveJSProperty(
      "validity.valid",
      false,
    );
  });

  test("newsletter rejects invalid email", async ({ page }) => {
    await page.goto("/");
    await page.locator('input[name="firstName"]').fill("Alaeddine");
    await page.locator('input[name="email"]').fill("adresse-invalide");
    await page.locator('input[name="consent"]').last().check();
    await page.getByRole("button", { name: /Recevoir la checklist/i }).click();

    await expect(page.locator('input[name="email"]')).toHaveJSProperty(
      "validity.valid",
      false,
    );
  });
});
